...
class Spinatra < Thor 
  ...
  method_option :verbose, :aliases => '-v', :desc => "Print directories and files created"
  def new(name)
    ...
    data["project_directories"].each do |dir|
      unless File.directory?(dir)
        puts "Creating directory \e[32m #{dir} \e[0m" if FileUtils.mkdir(dir) and verbose_output
      else
        puts "Skipped creating \e[31m #{dir} \e[0m" if verbose_output
      end
    end
    ...
    data['files'].each do |f|
      unless File.exist?(f)
        ...
        if source_file
          puts "Creating file \e[32m #{f} \e[0m" if verbose_output   
        end
        if data["#{f}"]
          data["#{f}"].each { |line| source_file.write line }
        end
      else
        puts "Skipped creating \e[31m #{f} \e[0m" if verbose_output
      end
    end
  end
